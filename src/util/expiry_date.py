...
def next_week_expiry(date: datetime, weekday: str):
    ...
    if days_until_weekday == 0:
        next_expire_date = date + datetime.timedelta(days=7)
    else:
        next_expire_date = date + datetime.timedelta(days=days_until_weekday)

    return next_expire_date


def next_month_expiry(date: datetime, weekday: str, ref_week_num=3):
    first_day_next_month = datetime.datetime(date.year if date.month != 12 else date.year + 1, (date.month % 12) + 1, 1)
    days_to_add = (weekday_mapping[weekday.upper()] - first_day_next_month.weekday() + 7) % 7 + (ref_week_num - 1) * 7
    next_expire_date = first_day_next_month + datetime.timedelta(days=days_to_add)
...
def closest_expiration_date(date: datetime, calendar=nyse_calendar):
    if calendar.valid_days(start_date=date, end_date=date).size > 0:
        ...
        return calendar.valid_days(start_date=date, end_date=date + datetime.timedelta(days=10))[0].replace(tzinfo=None).to_pydatetime()
...
def next_nth_trading_day(current: datetime, n: int):
    return nyse_calendar.valid_days(start_date=current, end_date=current+datetime.timedelta(days=365*3))[n-1].replace(tzinfo=None).to_pydatetime()
...
def closest_expiration_date(date: datetime, calendar=nyse_calendar):
    if calendar.valid_days(start_date=date, end_date=date).size > 0:
        ...
        return calendar.valid_days(start_date=date, end_date=date + datetime.timedelta(days=10))[0].replace(
